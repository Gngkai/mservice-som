...
public class aos_mkt_slogan_list  extends AbstractListPlugin {
    @Override
    ...
    private void aos_submit() {{
        try {
            ...
            while (iterator.hasNext()) {
                ...
                boolean reject = true;
                if (dy != null) {
                    if (dy.getString("aos_status").equals("翻译")) {
                        reject = false;
                    }
                }
                if (reject) {
    ...
    private void aos_batchgive(Object aos_user) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            DynamicObject aos_mkt_slogan = BusinessDataServiceHelper.loadSingle(id, "aos_mkt_slogan");
            String aos_userold = aos_mkt_slogan.getDynamicObject("aos_user").getPkValue().toString();
            String billno = aos_mkt_slogan.getString("billno");
            String  aos_status= aos_mkt_slogan.getString("aos_status");
            ...
            if (!"翻译,设计,海外翻译".contains(aos_status)) {
                this.getView().showTipNotification(billno + "只允许转办翻译,设计,海外翻译的单据!");
                return;
            }

            aos_mkt_slogan.set("aos_user", aos_user);
            ...
            MKTCom.SendGlobalMessage(((DynamicObject) aos_user).getPkValue() + "", aos_mkt_slogan + "",

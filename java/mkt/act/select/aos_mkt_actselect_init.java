...
//		long is_oversea_flag = aos_sal_sche_pub.get_lookup_values("AOS_YES_NO", "Y");
    ...
    private static Set<String> getStSku(Object pOuCode) {
        DynamicObjectCollection stSkuS = QueryServiceHelper.query("aos_base_stitem",
                "aos_itemid", new QFilter("aos_orgid.number",
                        QCP.equals, pOuCode)
                        .and("aos_type",QCP.equals,"AUTUMN").toArray());
        return stSkuS.stream().map(obj -> obj.getString("aos_itemid"))
                .collect(Collectors.toSet());
    }

    private static Set<String> getDistSku(Object pOuCode) {
        DynamicObjectCollection distSkuS = QueryServiceHelper.query("aos_mkt_distsku",
                "aos_itemid", new QFilter("aos_orgid.number",
                        QCP.equals, pOuCode).toArray());
        return distSkuS.stream().map(obj -> obj.getString("aos_itemid"))
                .collect(Collectors.toSet());
    }

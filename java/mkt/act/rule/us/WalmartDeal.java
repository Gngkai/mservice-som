...
public class WalmartDeal implements ActStrategy {
	...
	public void doOperation(DynamicObject object) throws Exception {
		...
		HashMap<String, Integer> Order = GenerateOrder(aos_orgid);// 30日销量
		...
		for (DynamicObject aos_mkt_actselect : aos_mkt_actselectS) {
			...
			if (!(ownWarehouseQuantity > 80 || platSaleDays >= 60)) {
				MktComUtil.putSyncLog(aos_sync_logS, aos_sku + " 自有仓库存>80；或平台仓可售天数≥60天；");
				continue;
			}
			...
			// 剔除过去30天销量为0的sku
			if (Order.get(aos_sku) == null ||Order.get(aos_sku)  ==0){
				MktComUtil.putSyncLog(aos_sync_logS, aos_sku + "剔除过去30天销量为0的sku");
				continue;
			}
			
			// TODO 剔除过去4期活动销量增量=0的sku
	...
	/** 近30日销量  **/
	private HashMap<String, Integer> GenerateOrder(String aos_orgid) {

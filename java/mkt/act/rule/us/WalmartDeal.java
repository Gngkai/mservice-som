...
public class WalmartDeal implements ActStrategy {
    ...
    public void doOperation(DynamicObject object) throws Exception {
        ...
        // 30日销量
        HashMap<String, Integer> order = generateOrder(aosOrgid);
        // 查询节日销售时间
        ...
        // 日志对象
        FndLog log = FndLog.init("WalmartDeal", LocalDate.now().toString());
        for (DynamicObject aosMktActselect : aosMktActselectS) {
            ...
            // 剔除过去30天销量为0的sku
            if (order.get(aosSku) == null || order.get(aosSku) == 0) {
                log.add(aosSku + " 剔除过去30天销量为0的sku");
                continue;
            }

            // 当前国别Review分数>=3或review分数==0
            if (!(aosStars.compareTo(BigDecimal.valueOf(0)) == 0 || aosStars.compareTo(BigDecimal.valueOf(3)) >= 0)) {
    ...
    /** 近30日销量 **/
    private HashMap<String, Integer> generateOrder(String aosOrgid) {
        ...
        DataSet orderDataSet = QueryServiceHelper.queryDataSet(this.getClass().getName() + "_GenerateOrderMonth",
            "aos_sync_om_order_r", "aos_org,aos_item_fid.number aos_itemnumber,aos_order_qty", filters, null);
        orderDataSet = orderDataSet.groupBy(new String[] {"aos_org", "aos_itemnumber"}).sum("aos_order_qty").finish();
        while (orderDataSet.hasNext()) {
            ...
            order.put(aosItemnumber, aosTotalQty);
        }
        orderDataSet.close();
        return order;
    }

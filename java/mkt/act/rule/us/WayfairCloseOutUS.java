...
public class WayfairCloseOutUS implements ActStrategy {
    ...
    public void doOperation(DynamicObject object) throws Exception {
        ...
        FndLog log = FndLog.init("WayfairCloseOutUS", LocalDate.now().toString());
        for (DynamicObject aosMktActselect : aosMktActselectS) {
            ...
            String aosContryentrystatus = itemObj.getString("aos_contryentrystatus");
            ...
            if (!"C".equals(aosContryentrystatus)) {
                log.add(aosSku + " 物料状态不为终止!");
                continue;
            }
            ...
            if ("REGULAR".equals(aosSeasonattr) || "SPRING-SUMMER-CONVENTIONAL".equals(aosSeasonattr)) {
                if (salDaysForAct < 120) {
                    log.add(aosSku + " 预计可售天数:" + salDaysForAct + " 常规品: 预计活动日可售天数 <120 ");
                    continue;
                }
            }
        ...
        // 按照库存金额倒序
        aosSalActplanentity.sort((lower, heigher) -> {

...
public class FlashSalesDE implements ActStrategy {
    @Override
    public void doOperation(DynamicObject object) throws Exception {
        ...
        // 日志
        DynamicObject aos_sync_log = ActUtil.getCommonLog("FlashSalesDE", aos_orgnum);
        ...
        for (DynamicObject obj : selectList) {
            ...
            if (!(noPlatQty >30 || platSaleDays >= 90)) {
                MKTCom.Put_SyncLog(aos_sync_logS, aos_sku + " 自有仓库存>30；或平台仓可售天数≥90天；");
                continue;
            }

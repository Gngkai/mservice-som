...
import kd.bos.isc.util.misc.Triple;
...
public class TrackerVipon implements ActStrategy {
	...
	public void doOperation(DynamicObject object) throws Exception {
		...
		try {
			...
			// 根据活动进行过滤
			list_filterItem = FilterItemByAct(billno, orgId, local_s.toString(), local_e.toString(), local_make,
					list_filterItem, aos_sync_logS, bidiMap_ietmID);
			...
			Map<String, BigDecimal> map_itemCost = calInventQty(orgId, list_filterItem, bidiMap_ietmID, map_stock,
					linkMap_inventPrice, aos_sync_logS);
			...
			// 营收和成本
			Pair<Map<String, BigDecimal>, Map<String, BigDecimal>> pair_cal = queryRevenueAndCost(orgId, orgNumber,
					shopid, list_filterItem, map_itemPirce, map_itemCost, map_itemActQty);
			...
			if (map_itemInfo.size() > 0) {
				...
				list_itemFields = itemInfo.getDynamicObjectType().getProperties().stream().map(pro -> pro.getName())
	...
	private static Map<String, String> getConventItem(Object orgID) {
		...
		DynamicObjectCollection dyc_ur = QueryServiceHelper.query("aos_sal_quo_ur", "aos_entryentity.aos_sku aos_sku",
				qfs);
		...
		DynamicObjectCollection dyc_st = QueryServiceHelper.query("aos_base_stitem", "aos_itemid",
				new QFilter[] { filter_org });
		List<String> list_itemid = dyc_st.stream().map(dy -> dy.getString("aos_itemid")).distinct()
				.collect(Collectors.toList());

		QFilter filter_id = new QFilter("id", QFilter.in, list_itemid);
		filter_org.__setProperty("aos_contryentry.aos_nationality");
		filter_season = new QFilter("aos_contryentry.aos_seasonseting", QFilter.in,
				seasonAttrDao.getConventionalProductSeason());
		StringJoiner str = new StringJoiner(",");
		str.add("id");
		DynamicObjectCollection dyc_item = QueryServiceHelper.query("bd_material", str.toString(),
				new QFilter[] { filter_org, filter_id, filter_season });
	...
	private static Map<String, String> getSeasonItem(Object orgID, DynamicObjectCollection aos_sync_logS) {
		...
		List<String> list_item = QueryServiceHelper
				.query("aos_sal_quo_sa", "aos_entryentity.aos_sku aos_sku",
						new QFilter[] { filter_from, filter_to, filter_org, filter_type })
		...
		DynamicObjectCollection dyc_item = QueryServiceHelper.query("bd_material", str.toString(),
				new QFilter[] { filter_org, filter_id });
	 ...
	 * @return filterItemid
	 */
	private static Map<String, Integer> FilterItemByStock(String orgID, BidiMap<String, String> bidiMap_ietmID,
		...
		Map<String, Integer> map_stockBefore = ActUtil.queryBalanceOverseasInventory(orgID,
				local_now.minusDays(15).toString(), list_itemNumber);
		// 获取当前库存
		...
		DynamicObject dy_stand = QueryServiceHelper.queryOne("aos_sal_quo_m_coe", "aos_value",
				new QFilter[] { filter_object, filter_org });
		int stand = 0;
	 ...
	 * @return filterItemid
	 */
	private static List<String> FilterItemByAct(String billno, String org, String date_s, String date_e,
			LocalDate local_make, List<String> list_ietm, DynamicObjectCollection aos_sync_logS,
			BidiMap<String, String> bidiMap_ietmID) {
		...
		DynamicObjectCollection dyc_otherAct = QueryServiceHelper.query("aos_act_select_plan",
				"aos_sal_actplanentity.aos_itemnum aos_itemnum", qfs);
		List<String> list_OtherActItem = dyc_otherAct.stream().map(dy -> dy.getString("aos_itemnum"))
				.collect(Collectors.toList());
		...
		qfs = new QFilter[] { filter_billno, filter_org, filter_actType, filter_make_e, filter_make_s,
				filter_headStatus, filter_rowStatus };
		DynamicObjectCollection dyc_theAct = QueryServiceHelper.query("aos_act_select_plan",
				"aos_sal_actplanentity.aos_itemnum aos_itemnum", qfs);
		List<String> list_sameAct = dyc_theAct.stream().map(dy -> dy.getString("aos_itemnum"))
				.collect(Collectors.toList());
	...
	private static Quad FilterItemByPrice(Object orgid, Object shopid, List<String> list_item,
		...
		Map<String, DynamicObject> map_AmazonPrice = SalUtil.get_shopPrice((String) orgid,
				dy_AmazonShop.getString("id"), list_item);
		// 物料成本(入库成本)
		Map<String, BigDecimal> map_cost = common.sal.util.SalUtil.get_NewstCost((String) orgid, list_item);
		// 最低快递费
		Map<String, BigDecimal> map_fee = common.sal.util.SalUtil.getMinExpressFee((String) orgid, (String) shopid,
				list_item);
		// 记录原价和活动价格
		...
		for (String item : list_item) {
			...
			str.add("成本：  " + big_cost);
			BigDecimal big_price = big_fee.add(big_Amprice.multiply(BigDecimal.valueOf(0.15)))
					.add(big_cost.multiply(BigDecimal.valueOf(0.3)));
			...
			if (big_discount.compareTo(big_price) < 0) {
				...
				if (big_Amprice.doubleValue() != 0) {
					BigDecimal bid_increaseRate = (bid_increase.subtract(big_Amprice)).divide(big_Amprice, 2,
	...
	private static BidiMap<String, String> mapItemIDToNumber(List<String> list_item) {
		...
		DynamicObjectCollection dyc = QueryServiceHelper.query("bd_material", "id,number",
				new QFilter[] { filter_number });
	 ...
	 */
	private static Map<String, BigDecimal> calInventQty(Object orgid, List<String> list_filterItem,
			BidiMap<String, String> bidiMap_idToNumber, Map<String, Integer> map_itemStock,
			LinkedHashMap<String, BigDecimal> linkMap_re, DynamicObjectCollection aos_sync_logS) {
		...
		List<Map.Entry<String, BigDecimal>> list_re = map_re.entrySet().stream().sorted((ent1, ent2) -> {
	...
	private static List<String> fillItem(List<String> list_filterItem, Map<String, String> map_conventItem,
			Map<String, String> map_seasonItem, Map<String, Integer> map_cateStand,
			Map<String, List<String>> map_cate) {
	...
	private static Map<String, Integer> calItemActQty(List<String> list_item,
			BidiMap<String, String> map_itemIdToNumber, Map<String, Integer> map_stock) {
		Map<String, Integer> map_re = new HashMap<>();
		for (String itemid : list_item) {
			String itemNumber = map_itemIdToNumber.get(itemid);
			int stock = map_stock.getOrDefault(itemNumber, 0);
			stock = BigDecimal.valueOf(stock).multiply(BigDecimal.valueOf(0.1)).setScale(0, BigDecimal.ROUND_HALF_UP)
					.intValue();
	...
	private static Pair<Map<String, BigDecimal>, Map<String, BigDecimal>> queryRevenueAndCost(Object orgid,
			String orgNumber, Object shopid, List<String> list_item, Map<String, BigDecimal[]> map_price,
			Map<String, BigDecimal> map_cost, Map<String, Integer> map_actQty) {
		// 获取快递费
		...
		DynamicObject dy_shop = QueryServiceHelper.queryOne("aos_sal_shop", "aos_plat_rate,aos_vat",
				new QFilter[] { filter_shop });
		...
		// vat
		if (dy_shop != null && dy_shop.get("aos_vat") != null
				&& dy_shop.getBigDecimal("aos_vat").compareTo(BigDecimal.ZERO) > 0)
	...
	private static Map<String, String> QueryTypedatail(String aos_orgnum, List<String> list_itemID) {
		...
		return selectList.stream().filter(dy -> list_itemID.contains(dy.getString("aos_sku"))).collect(Collectors
	...
	public static Map<String, List<String>> getItemCate(List<String> list_item) {
		...
		DynamicObjectCollection dyc_data = QueryServiceHelper.query("bd_materialgroupdetail",
				"group.name group,material", new QFilter[] { filter_item, filter_standard });
		for (DynamicObject dy : dyc_data) {
			List<String> list_cate = Arrays.stream(dy.getString("group").split(",")).collect(Collectors.toList());

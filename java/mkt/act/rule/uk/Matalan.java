...
public class Matalan implements ActStrategy {
    ...
    public void doOperation(DynamicObject object) throws Exception {
        ...
        // 销售订单数据 平台30天销量
        HashMap<String, Integer> OrderData = ActUtil.GenOrderData(aos_orgid, aos_channelid);

        DynamicObject aos_sync_log = ActUtil.getCommonLog("MatalanUK", aos_orgnum);
        ...
        // 获取亚马逊价格
        Map<String, DynamicObject> amazonPrice = ActUtil.getAmazonPrice(aos_orgid, itemFilterList);
        // 查询节日销售时间
        ...
        for (DynamicObject obj : firstFilterList) {
            ...
            // asin
            if (priceObj == null) {
                priceObj = amazonPrice.get(aos_sku);
                if (priceObj == null) {
                    MktComUtil.putSyncLog(aos_sync_logS, aos_sku + "取不到价格");
                    continue;
                }
            }
            ...
            if ("UK-Ryman".equals(aos_shopnum) && "Leaftlet".equals(aos_acttypenum)) {
                if (OrderData.get(aos_sku) == null || OrderData.get(aos_sku) == 0){
                    MktComUtil.putSyncLog(aos_sync_logS, aos_sku + "平台过去30天销量为0");
                    continue;
                }
            }
            // 预计活动日可售天数
...
//            itemObject.put("aos_reviewqty", aos_review);// review个数
        ...
        if ("UK-Ryman".equals(aos_shopnum) && "Leaftlet".equals(aos_acttypenum)) {
            // 分组+剔除
            Map<String, List<DynamicObject>> categoryMap = new HashMap<>();
            ...
            for (DynamicObject lineObject : aos_sal_actplanentity1) {
                String aos_category_stat1 = lineObject.getString("aos_category_stat1");
                List<DynamicObject> jsonObjects = categoryMap.computeIfAbsent(aos_category_stat1, k -> new ArrayList<>());
            ...
            for (String key : categoryMap.keySet()) {
                aos_sal_actplanentity1.addAll(categoryMap.get(key));
            }

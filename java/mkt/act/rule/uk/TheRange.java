...
public class TheRange implements ActStrategy {
    ...
    public void doOperation(DynamicObject object) throws Exception {
        ...
        FndLog log = FndLog.init("TheRangeUK", aosOrgnum);
        ...
        for (DynamicObject obj : firstFilterList) {
            ...
            boolean cond =
                ("Banner".equals(aosActtypenum) && (orderData.get(aosSku) == null || orderData.get(aosSku) == 0));
            if (cond) {
                log.add(aosSku + "Banner额外条件 平台过去30天销量为0");
                continue;
            }
            // 预计活动日可售天数
        ...
        if (BANNER.equals(aosActtypenum)) {
            // 物料及其小类
            ...
            for (List<DynamicObject> list : collect) {
                ...
                if (groupCount == collect.size()) {
                    ...
                    BigDecimal rate = BigDecimal.valueOf(size).divide(BigDecimal.valueOf(sum), 3, RoundingMode.HALF_UP)
                        .multiply(BigDecimal.valueOf(120)).setScale(0, RoundingMode.HALF_UP);
                    // 计数 如果达到组别比例就不再取值
                    int z = 0;
                    for (DynamicObject obj : list) {

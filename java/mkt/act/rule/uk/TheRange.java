...
public class TheRange implements ActStrategy {
	@Override
	public void doOperation(DynamicObject object) throws Exception {
		...
		DynamicObject aos_sync_log = ActUtil.getCommonLog("TheRangeUK", aos_orgnum);
		...
		for (DynamicObject obj : firstFilterList) {
			...
			// Banner额外条件平台过去30天销量>0
			if ("Banner".equals(aos_acttypenum) && (OrderData.get(aos_sku) == null || OrderData.get(aos_sku) == 0)) {
				MktComUtil.putSyncLog(aos_sync_logS, aos_sku + "Banner额外条件 平台过去30天销量为0");
				continue;
			}
...
//            itemObject.put("aos_reviewqty", aos_review);// review个数
		...
		if ("Banner".equals(aos_acttypenum)) {
			// 物料及其小类
			...
			for (List<DynamicObject> list : collect) {
				...
				if (groupCount == collect.size()) {
					...
					int size = list.size();
					// 比例
					BigDecimal rate = BigDecimal.valueOf(size).divide(BigDecimal.valueOf(sum), 3, BigDecimal.ROUND_HALF_UP).multiply(BigDecimal.valueOf(120)).setScale(0, BigDecimal.ROUND_HALF_UP);
					// 计数 如果达到组别比例就不再取值
					int z = 0;
					for (DynamicObject obj : list) {

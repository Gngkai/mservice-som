...
public class Mano implements ActStrategy {
    ...
    public void doOperation(DynamicObject object) throws Exception {
        ...
        FndLog log = FndLog.init("ManoUK", aosOrgnum);
        ...
        Map<String, Integer> manoZeroSalesMap = ActUtil.queryZeroSalesMano(aosOrgid, aosShopid);
        // 选择满足库存条件的sku
        ...
        for (DynamicObject obj : firstFilterList) {
            ...
            if ("UK-ManoMano".equals(aosShopnum) && "Coupon".equals(aosActtypenum)) {
                Integer sales = manoZeroSalesMap.getOrDefault(aosItemid, 0);
                if (sales == 0) {
                    log.add(aosSku + "Coupon额外条件 mano过去30天销量=0");
                    continue;
                }
            }

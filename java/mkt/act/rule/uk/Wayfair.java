...
public class Wayfair implements ActStrategy {
    @Override
    public void doOperation(DynamicObject object) throws Exception {
        ...
        Set<String> apartFromAmzAndEbayItem =
            ActUtil.queryApartFromAmzAndEbayItem(aos_orgid, new String[] {"AMAZON", "EBAY"}, start);
        // 自有仓库可用量大于等于30的物料
        ...
        DynamicObject aos_sync_log = ActUtil.getCommonLog("WayfairUK", aos_orgnum);
        ...
        for (DynamicObject obj : firstFilterList) {
            ...
            if ("SPRING".equals(aos_seasonattr) || "SPRING_SUMMER".equals(aos_seasonattr)
                ...
                if (salDaysForAct < betweenDays) {
                    MktComUtil.putSyncLog(aos_sync_logS,
                        aos_sku + "春夏品: 预计活动日可售天数=" + salDaysForAct + " 季末-预计活动日=" + betweenDays);
                    continue;
                }
            }
            if ("AUTUMN_WINTER".equals(aos_seasonattr) || "WINTER".equals(aos_seasonattr)) {
                ...
                if (salDaysForAct < betweenDays) {
                    MktComUtil.putSyncLog(aos_sync_logS,
                        aos_sku + "秋冬品: 预计活动日可售天数=" + salDaysForAct + " 季末-预计活动日=" + betweenDays);
                    continue;
                }
            }
            ...
            if (dateArr != null) {
                ...
                if (salDaysForAct < betweenDays) {
                    MktComUtil.putSyncLog(aos_sync_logS,
                        aos_sku + "节日品: 预计活动日可售天数=" + salDaysForAct + " 季末-预计活动日=" + betweenDays);
                    continue;
                }
            }

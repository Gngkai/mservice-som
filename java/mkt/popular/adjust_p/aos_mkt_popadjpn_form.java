...
public class aos_mkt_popadjpn_form extends AbstractFormPlugin implements ItemClickListener, RowClickEventListener {
    ...
    private static final Tracer tracer =
            MmsOtelUtils.getTracer(aos_mkt_popbudgetp_form.class, RequestContext.get());
            ...
            for (int i = 0; i < RowCount; i++) {
                ...
                switch (sign) {
                    case "aos_multipy":
                        ...
                        break;
                    case "aos_divide":
                        result = aos_lastprice.divide(aos_adjustrate, 2, RoundingMode.HALF_UP);
                        break;
                    case "aos_add":
    ...
    private void InitData(ArrayList<QFilter> arr_q) {
        ...
        String SelectField = "aos_detailentry.aos_poptype aos_poptype,"
                ...
                "aos_detailentry.aos_offline aos_offline," +
                "aos_detailentry.aos_special aos_special";
        ...
        for (DynamicObject aos_mkt_popadjustp_data : aos_mkt_popadjustp_dataS) {
            ...
            this.getModel().setValue("aos_offline", aos_mkt_popadjustp_data.get("aos_offline"), i);
            this.getModel().setValue("aos_special", aos_mkt_popadjustp_data.get("aos_special"), i);
            ...
            if (aos_lastprice.compareTo(BigDecimal.ZERO) != 0) {
                this.getModel().setValue("aos_rate",
                        aos_adjprice.divide(aos_lastprice, 2, RoundingMode.HALF_UP).subtract(BigDecimal.valueOf(1)),
                        i);
            }
            i++;
        }
    ...
    public void propertyChanged(PropertyChangedArgs e) {
        ...
        if (name.equals("aos_adjprice")) {
            ...
            if (rowIndex >= 0) {
                ...
                if (aos_lastprice.compareTo(BigDecimal.ZERO) != 0) {
                    this.getModel().setValue("aos_rate", aos_adjprice.divide(aos_lastprice, 2, RoundingMode.HALF_UP)
                            .subtract(BigDecimal.valueOf(1)), rowIndex);
                }
                if (aos_bidsuggest.compareTo(BigDecimal.ZERO) != 0) {
                    this.getModel().setValue("aos_ratio", aos_adjprice.divide(aos_bidsuggest, 2, RoundingMode.HALF_UP));
                }
    ...
    public void afterBindData(EventObject e) {
        ...
        String aos_status = String.valueOf(this.getView().getParentView().getModel().getValue("aos_status"));

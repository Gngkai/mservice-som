 ...
 * 布景标准库初始化
 */
public class AosMktViewInit extends AbstractTask {
...
//        HashMap<String, String> cateUser = getCateUser();
        ...
        List<DynamicObject> aosMktViewS = new ArrayList<>();
        int i = 0;
        while (groupS.hasNext()) {
            ...
            if (FndGlobal.IsNotNull(stdCategory) && stdCategory.contains(aosCategory1 + "~" +
                ...
                continue;
            DynamicObject aosMktView = BusinessDataServiceHelper.newDynamicObject("aos_mkt_viewstd");
            aosMktView.set("aos_category1_name", aosCategory1);
            aosMktView.set("aos_category2_name", aosCategory2);
            aosMktView.set("aos_category3_name", aosCategory3);
            aosMktView.set("aos_itemnamecn", itemName);
            aosMktView.set("aos_status", "待制作");
//            aosMktDesign.set("aos_user", cateUser.get(userKey));
            aosMktViewS.add(aosMktView);
            if (aosMktViewS.size() >= 5000 || !groupS.hasNext()) {
                SaveServiceHelper.save(aosMktViewS.toArray(new DynamicObject[0]));
                aosMktViewS.clear();
            }
        }
        groupS.close();
    ...
    private static Set<String> getStdCategory() {
        DynamicObjectCollection list = QueryServiceHelper.query("aos_mkt_viewstd",
                "aos_category1_name," +
                        "aos_category2_name," +
                        "aos_category3_name," +
                        "aos_itemnamecn",

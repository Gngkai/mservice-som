...
public class aos_mkt_syc_shipmentdate extends AbstractTask {
    ...
    public static void SyncPhotoReq(){
        ...
        DynamicObject[] dyc_req = BusinessDataServiceHelper.load("aos_mkt_photoreq", str.toString(), new QFilter[]{filter_status});
        //获取出运日期
        ...
        for (DynamicObject dy_req : dyc_req) {
            ...
            if (dy_req.getBoolean("aos_newitem") || dy_req.getBoolean("aos_newvendor")){
                if (dy_req.get("aos_itemid")!=null) {
                    ...
                    DynamicObjectCollection dyc = iteminfo.GetShipDate(item, aos_ponumber,new String[]{"aos_overseasdate"});
                    if (dyc.size()>0) {
    ...
    private static void SyncRcv(String fid,Date shipDate ){
        ...
        DynamicObject[] dyc = BusinessDataServiceHelper.load("aos_mkt_rcv", str.toString(), new QFilter[]{filter_Id});
    ...
    public static void SyncDesignreq(String fid,Date shipDate){
        ...
        DynamicObject[] dyc_design = BusinessDataServiceHelper.load("aos_mkt_designreq", str.toString(), new QFilter[]{filter_source});
        List<DynamicObject> list_save = new ArrayList<>(5000);
    ...
    public static void SyncPhotoList(String billno,Date shipDate){
        ...
        str.add("id");
        str.add("aos_shipdate");
        DynamicObject[] dyc = BusinessDataServiceHelper.load("aos_mkt_photolist", str.toString(), new QFilter[]{filter_bill});
        for (DynamicObject dy : dyc) {
            dy.set("aos_shipdate",shipDate);
        }
        SaveServiceHelper.update(dyc);

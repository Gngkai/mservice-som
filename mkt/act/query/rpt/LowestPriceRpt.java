...
public class LowestPriceRpt extends AbstractFormPlugin implements BeforeF7SelectListener {
    ...
    private void inquire() {
        ...
        // 分组字段
        String[] groupByField = {"aos_orgid", "aos_itemid"};
        ...
        // 最低成交价查询字段
        String selectFields1 = "aos_org aos_orgid, " +
                "aos_item_fid aos_itemid," +
                "aos_trans_price aos_lwsttransprice";
        ...
        // 活动次数查询字段
        String selectFields2 = "aos_nationality aos_orgid, " +
        ...
        String selectFields3 = "aos_entryentity.aos_orgid aos_orgid, " +
        ...
        // 只填了国别和渠道
            ...
            selectFields1 = "aos_org aos_orgid, " +
                    "aos_item_fid aos_itemid," +
            ...
            selectFields2 = "aos_nationality aos_orgid, " +
                    "aos_channel aos_platformid, " +
                    "aos_sal_actplanentity.aos_itemnum aos_itemid," +
                    "1 as aos_acttimes";

            fieldList.add("aos_platformid");
            groupByField = new String[]{"aos_orgid", "aos_platformid", "aos_itemid"};
        } else if (4 == queryType) {
            // 最低成交价
            qFilter1.and(new QFilter("aos_org", QCP.equals, aos_orgid));
            qFilter1.and(new QFilter("aos_platformfid", QCP.equals, aos_platformid));
        ...
        dataSet = dataSet.join(dataSet3, JoinType.LEFT)
                .on("aos_orgid", "aos_orgid")
                .on("aos_itemid", "aos_itemid")
                .select(fieldList.toArray(new String[0]))
                .finish();
        ...
        if (3 == queryType) {
            ...
            dataSet = dataSet.where("aos_platformid = " + aos_platformid);
            dataSet = dataSet.where("aos_shopid = " + aos_shopid);
        }
    ...
    private DataSet joinDataSet(DataSet dataSet1, DataSet dataSet2, int queryType, List<String> fieldList) {
        ...
        if (1 == queryType || 2 == queryType) {
            ...
            joinDataSet = joinDataSet
                    .on("aos_orgid", "aos_orgid")
                    .on("aos_platformid", "aos_platformid")
                    .on("aos_itemid", "aos_itemid")
                    .select(fieldList.toArray(new String[0]));
        } else {
            joinDataSet = joinDataSet
    ...
    private DataSet queryActivityTimes(String selectFields, QFilter qFilter, String[] groupByField) {
        ...
        DataSet dataSet = QueryServiceHelper.queryDataSet(algoKey, BILL_ACTIVITY_PLAN, selectFields, qFilter.toArray(), null);
        return dataSet
    ...
    private DataSet queryLowestTransPrice(String selectFields, QFilter qFilter, String[] groupByField) {
        ...
        DataSet dataSet = QueryServiceHelper.queryDataSet(algoKey, BILL_ORDER_LINE, selectFields, qFilter.toArray(), null);
        return dataSet
                .groupBy(groupByField)
                .min("aos_lwsttransprice")
                .finish();
    }
    ...
    private DataSet queryLowestPricing(String selectFields, QFilter qFilter, String[] groupByField) {
        ...
        DataSet dataSet = QueryServiceHelper.queryDataSet(algoKey, BILL_LOWEST_PRICING, selectFields, qFilter.toArray(), null);
        return dataSet
                .groupBy(groupByField)
                .min("aos_lwstpricing")
                .finish();

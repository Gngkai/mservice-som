...
//		long is_oversea_flag = aos_sal_sche_pub.get_lookup_values("AOS_YES_NO", "Y");
	...
	public static void do_operate(Map<String, Object> params) {
		...
		for (DynamicObject bd_material : bd_materialS) {
			...
			// 2.0常规品 滞销
				...
				aos_typedetail = MKTCom.Get_RegularUn(aos_orgnumber, availableDays, R,saleout);
	...
	private static List<String> getNewestUnsaleDy(String orgNumber){
		//获取最新一期的时间
		DynamicObject dy_date = QueryServiceHelper.queryOne("aos_sal_quo_ur", "max(aos_makedate) as aos_makedate", null);
		if (dy_date==null||dy_date.get("aos_makedate")==null)
			return new ArrayList<>();
		else{
			LocalDate localDate = dy_date.getDate("aos_makedate").toInstant().atZone(ZoneId.systemDefault()).toLocalDate();
			...
			return QueryServiceHelper.query("aos_sal_quo_ur", "aos_entryentity.aos_sku aos_sku", qfs)
					.stream()

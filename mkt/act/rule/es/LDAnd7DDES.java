...
public class LDAnd7DDES implements ActStrategy {
    @Override
    public void doOperation(DynamicObject object) throws Exception {
        ...
        DynamicObject aos_sync_log = ActUtil.getCommonLog("LDAnd7DDES", aos_orgnum);
        ...
        for (DynamicObject obj : firstFilterList) {
            ...
            if (ownWarehouseQty == 0) {
                MKTCom.Put_SyncLog(aos_sync_logS, aos_sku + "自有仓库为0");
                continue;
            }
            if (!(ownWarehouseQty > 30 || platSaleDays >= 90)) {
            ...
            if (dates != null) {
                ...
                if ((float)sum / (float) 2 <= 3) {
                    MKTCom.Put_SyncLog(aos_sync_logS, aos_sku + " 近3个月内，最近2次LD/7DD均销量≤3");
                    continue;
                }
            }

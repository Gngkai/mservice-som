...
public class ThemeEventES implements ActStrategy {
    @Override
    public void doOperation(DynamicObject object) throws Exception {
        ...
        DynamicObject aos_sync_log = ActUtil.getCommonLog("VCES", aos_orgnum);
        ...
        for (DynamicObject obj : selectList) {
            ...
            if (apartFromAmzItem.contains(aos_sku)) {
                MKTCom.Put_SyncLog(aos_sync_logS, aos_sku + "剔除除亚马逊外活动日过去30天已提报平台活动个数＞3的sku");
                continue;
            }

            if (samePeriodActivitiesSet.contains(aos_sku)) {
                MKTCom.Put_SyncLog(aos_sync_logS, aos_sku + "剔除除剔除同国别同期活动");
                continue;
            }

            if (daysBeforeAndAfterActItem.contains(aos_sku)) {
                MKTCom.Put_SyncLog(aos_sync_logS, aos_sku + " 剔除同店铺前后30天已提报活动的SKU");
                continue;
            }
        ...
        for (DynamicObject obj : firstFilterList) {
            ...
            if (!(ownWarehouseQty > 50)) {
                MKTCom.Put_SyncLog(aos_sync_logS, aos_sku + "自有仓库<=50 自有仓库库存数量:" + ownWarehouseQty);
                continue;
            }
...
//            itemObject.put("aos_reviewqty", aos_review);// review个数
        ...
        if (("ES-Worten".equals(aos_shopnum) && ("Theme Event".equals(aos_acttypenum) || "Mega Noite".equals(aos_acttypenum)))) {
            // 按销售组别分组
        ...
        } else if (("ES-Sprinter".equals(aos_shopnum) && "Theme Event".equals(aos_acttypenum))
                || ("ES-Sprinter PT".equals(aos_shopnum) && "Theme Event".equals(aos_acttypenum))
                || ("ES-MediaMarkt ES".equals(aos_shopnum) && "Theme Event".equals(aos_acttypenum))) {
            // 按销售组别分组
            Map<String, List<DynamicObject>> groupMap = groupBy(aos_orgid, itemFilterList, objectArr);
            // 计算销售组比例
            Map<String, Float> rateMap = ActUtil.calGroupItemQtyRate(groupMap);
            ...
            float incrementRate = 0.5f;
            resultMap = ActUtil.fillResult(groupMap, groupActIncrementMap, groupInvCostAmtMap, rateMap, totalQty, incrementRate);
        } else if ("ES-PC COMPONENTES Y MULTIMEDIA SLU".equals(aos_shopnum) && "Theme Event".equals(aos_acttypenum)) {
            // 按销售组别分组
            Map<String, List<DynamicObject>> groupMap = groupBy(aos_orgid, itemFilterList, objectArr);
            // 计算销售组比例
            Map<String, Float> rateMap = ActUtil.calGroupItemQtyRate(groupMap);
            ...
            float incrementRate = 0.5f;
            resultMap = ActUtil.fillResult(groupMap, groupActIncrementMap, groupInvCostAmtMap, rateMap, totalQty, incrementRate);
        } else if ("ES-Worten ES".equals(aos_shopnum) && ("Theme Event".equals(aos_acttypenum) || "WD".equals(aos_acttypenum))) {
            Map<String, List<DynamicObject>> groupMap = groupWortenES(objectArr);
            // 计算销售组比例
            ...
            resultMap = ActUtil.fillResult(groupMap, groupActIncrementMap, groupInvCostAmtMap, rateMap, totalQty, incrementRate);
        } else if ("ES-Carrefour".equals(aos_shopnum) && "Weekend Sales".equals(aos_acttypenum)) {
            Map<String, List<DynamicObject>> groupMap = groupCarrefour(objectArr);
            // 计算销售组比例
            Map<String, Float> rateMap = new HashMap<>();
            // 按活动增量倒序
    ...
    private Map<String, List<DynamicObject>> groupCarrefour(DynamicObjectCollection objectArr) {
        Map<String, List<DynamicObject>> groupMap = new HashMap<>();
        // 分组
        for (DynamicObject object1 : objectArr) {
            String aos_category_stat1 = object1.getString("aos_category_stat1");
            // 小类
            if ("宠物用品".equals(aos_category_stat1)
                    || "庭院&花园".equals(aos_category_stat1)
                    || "婴儿产品&玩具及游戏".equals(aos_category_stat1)) {
                List<DynamicObject> groupList = groupMap.computeIfAbsent(aos_category_stat1, k -> new ArrayList<>());
                groupList.add(object1);
            }
    ...
    private Map<String, List<DynamicObject>> groupWortenES(DynamicObjectCollection objectArr) {
        Map<String, List<DynamicObject>> groupMap = new HashMap<>();
        // 分组
        for (DynamicObject object1 : objectArr) {
            String aos_category_stat1 = object1.getString("aos_category_stat1");
            // 小类
            if ("居家系列".equals(aos_category_stat1)
                    || "宠物用品".equals(aos_category_stat1)
                    || "婴儿产品&玩具及游戏".equals(aos_category_stat1)
                    || "运动&娱乐&汽配".equals(aos_category_stat1)) {
                List<DynamicObject> groupList = groupMap.computeIfAbsent(aos_category_stat1, k -> new ArrayList<>());
                groupList.add(object1);
            }
        }

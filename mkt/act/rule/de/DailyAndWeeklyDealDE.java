...
public class DailyAndWeeklyDealDE implements ActStrategy {
    @Override
    public void doOperation(DynamicObject object) throws Exception {
        ...
        Set<String> beforeAfter30Set = ActUtil.queryCullActTypeItem(aos_orgid, start, new String[]{"WD", "DD", "Flash Seals"}, 30);
        ...
        for (DynamicObject obj : selectList) {
            ...
            if (beforeAfter30Set.contains(aos_itemid)) {
                MKTCom.Put_SyncLog(aos_sync_logS, aos_sku + " 剔除活动日前后30天已提报WD、DD、Flash Seals的sku");
                continue;
            }
            ...
            if (beforeAfter30Set.contains(aos_itemid)) {
                MKTCom.Put_SyncLog(aos_sync_logS, aos_sku + " 剔除①预计活动日前后30天，活动类型为DOTD/LD/7DD/Tracker，且状态为正常的在线ID");
                continue;
            }
...
//            itemObject.put("aos_reviewqty", aos_review);// review个数
        ...
        Map<String, List<DynamicObject>> resultMap = ActUtil.fillResult(groupMap, groupActIncrementMap, groupInvCostAmtMap, rateMap, totalQty, incrementRate);

...
public class ManoDE implements ActStrategy {
    @Override
    public void doOperation(DynamicObject object) throws Exception {
        ...
        Set<String> daysBeforeAndAfterActItem = ActUtil.query30DaysBeforeAndAfterActItem(aos_orgid, start, 30);
        ...
        // 剔除同店铺同期活动
        Set<String> samePeriodActivitiesSet = ActUtil.querySamePeriodActivities(aos_orgid, aos_shopid, start, end);
        ...
        for (DynamicObject obj : selectList) {
            ...
            if (!(noPlatQty > 30)) {
                MKTCom.Put_SyncLog(aos_sync_logS, aos_sku + " 自有仓库存>30；自有仓库数量:" + noPlatQty);
                continue;
            }
...
//            itemObject.put("aos_reviewqty", aos_review);// review个数
        ...
        if (("ES-Worten".equals(aos_shopnum) && ("Theme Event".equals(aos_acttypenum) || "Mega Noite".equals(aos_acttypenum)))) {
            // 按销售组别分组
            Map<String, List<DynamicObject>> groupMap = ActUtil.groupBy(aos_orgid, itemFilterList, objectArr);
            // 计算销售组比例

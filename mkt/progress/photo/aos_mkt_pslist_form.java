...
public class aos_mkt_pslist_form extends AbstractBillPlugIn implements ItemClickListener {
	...
	/** 初始化数据 **/
	private void InitData(QFilter item_Qf) {
		...
		DynamicObjectCollection aos_mkt_pslistS = QueryServiceHelper.query("aos_mkt_pslist", SelectColumn,
				new QFilter[] { item_Qf });
		this.getModel().deleteEntryData("aos_entryentity");
		;
		int i = 0;
		for (DynamicObject aos_mkt_pslist : aos_mkt_pslistS) {
			this.getModel().batchCreateNewEntryRow("aos_entryentity", 1);
			this.getModel().setValue("aos_itemid", aos_mkt_pslist.get("aos_itemid"), i);
			this.getModel().setValue("aos_itemname", aos_mkt_pslist.get("aos_itemname"), i);
			this.getModel().setValue("aos_designer", aos_mkt_pslist.get("aos_designer"), i);
			this.getModel().setValue("aos_shipdate", aos_mkt_pslist.get("aos_shipdate"), i);
			this.getModel().setValue("aos_arrivaldate", aos_mkt_pslist.get("aos_arrivaldate"), i);
			this.getModel().setValue("aos_reqno", aos_mkt_pslist.get("aos_reqno"), i);
			this.getModel().setValue("aos_emstatus", aos_mkt_pslist.get("aos_emstatus"), i);
			this.getModel().setValue("aos_status", aos_mkt_pslist.get("aos_status"), i);
			this.getModel().setValue("aos_epi", aos_mkt_pslist.get("aos_epi"), i);
			this.getModel().setValue("aos_initdate", aos_mkt_pslist.get("aos_initdate"), i);
			this.getModel().setValue("aos_startdate", aos_mkt_pslist.get("aos_startdate"), i);
			this.getModel().setValue("aos_enddate", aos_mkt_pslist.get("aos_enddate"), i);
			this.getModel().setValue("aos_sourceid", aos_mkt_pslist.get("aos_sourceid"), i);
			this.getModel().setValue("aos_id", aos_mkt_pslist.get("id"), i);
			// 入库日期
			this.getModel().setValue("aos_rcvdate", aos_mkt_pslist.get("aos_rcvdate"), i);
			i++;
		}
	}
	...
	private void aos_save() throws FndError {
		...
		for (int i = 0; i < SelectRow.length; i++) {
			...
			Object aos_startdate = this.getModel().getValue("aos_startdate", SelectRow[i]);
			Object aos_enddate = this.getModel().getValue("aos_enddate", SelectRow[i]);

			aos_mkt_pslist.set("aos_epi", this.getModel().getValue("aos_epi", SelectRow[i]));
			aos_mkt_pslist.set("aos_startdate", aos_startdate);
			aos_mkt_pslist.set("aos_enddate", aos_enddate);
